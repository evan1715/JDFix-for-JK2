...
void respawn( gentity_t *ent ) {
	...
	//JediDog: sleep
	if (ent->client->pers.Sleep == qtrue) {
...
void ClientUserinfoChanged( int clientNum ) {
	...
	//JediDog: Galak_Mech kicker
/*	ModelCrash = Info_ValueForKey (userinfo, "name");
	if (strcmp(Info_ValueForKey (userinfo, "model"), "galak_mech") == 0) {
		trap_SendConsoleCommand(EXEC_INSERT, va("clientkick \"%d\"\n", clientNum));
		trap_SendServerCommand(-1, va("print\"%s ^5is being kicked for using the galak_mech model exploit.\n", ModelCrash));
		G_LogPrintf("%s was kicked because the client used the model galak_mech.", ModelCrash);
	}
*/
/*	//JediDog: Galak Mech Fix
	if (strcmp(Info_ValueForKey (userinfo, "model"), "galak_mech")== 0) {
		return;
	}
	//end
...
//	static const unsigned char validChars[] = " ~QqWwEeRrTtYyUuIiOoPpAaSsDdFfGgHhJjKkLlZzXxCcVvBbNnMm1234567890<>?,./';:][{}`-=!@#$^&*()_+|";
//	unsigned char jdIllegalName[24];
	char jdPortlessIP[24];
//	#define jdPB "cl_punkbuster"
//	#define jdCLG "cl_guid"
//	int i;
//	int c;
//	int j;
	...
	//JediDog: Added this from my mod & Gamall mod, I like this idea too much to deny it :D
	if (jd_CloseServer.integer !=0 && firstTime) {
		G_LogPrintf("Access denied because the server is closed\n");
		if (jd_CloseServer.integer !=0) {
			value = Info_ValueForKey (userinfo, "name");
			if (jd_Messages.integer !=0) {
				trap_SendServerCommand(-1, va("print\"^2Closed server: ^5Client ^7<^7%s^7>^5 was denied access^2.\n\"", value));
			}
		}
		return "The server is closed right now, come back later. :)";
	}//JediDog: End.
	...
	/* BEGINNING OF JEDIDOG FIXES */
	//JediDog: get ip address
	//JEDIDOG: NOTE: DISABLED THIS PORTLESS IP CORE BECAUSE WE DON'T NEED IT
/*	strcpy(jdPortlessIP, Info_ValueForKey (userinfo, "ip"));
	for (i = 0 ; i < 24 ; i++) {
		if (jdPortlessIP[i] == ':') {
			jdPortlessIP[i] = 0;
			break;
		}
	}//end of fake player core
	// JediDog: beginning of illegal name fix
	//JEDIDOG: NOTE: DISABLED THIS ANTI NAME CRASH TO SUBSTITUTE IT FOR THE ONE IN USERINFO
	strcpy((char*)jdIllegalName, Info_ValueForKey (userinfo, "name"));
	for(i = 0; (c = jdIllegalName[i]); i++) {
		for(j = 0; validChars[j]; j++) {
			if(c == validChars[j]) break;
		}// if illegal character name glitch detection is activated, check user info
//		if (jd_antiNameCrash.integer != 0) {
			if (!validChars[j]) {
//				G_LogPrintf("JEDIDOG FIX: The IP Address %s has Illegal Characters in their name. Possible name glitch/crash attempt. The client has been denied access.\n", jdPortlessIP);
				return "Illegal characters detected in name, reconnect as a different name.";
			}
	}
//	}// END OF ILLEGAL NAME FIX
*/
	// Begin Fake Players Detection
//	if (jd_FakePlayerDetection.integer != 0) {
