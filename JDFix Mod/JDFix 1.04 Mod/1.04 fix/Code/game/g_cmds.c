...
char	*ConcatArgs( int start ) {
	...
	for ( i = start ; i < c ; i++ ) {
		...
		tlen = strlen( arg );
		//JediDog: added this for anti 'msgboom/msgbof'
		if ( len + tlen >= 896 ) {
			break;
		}
...
void Cmd_Kill_f( gentity_t *ent ) {
	...
	//JediDog: sleeping clients cannot kill themselves
...
void SetTeam( gentity_t *ent, char *s ) {
	...
	//JediDog: Got rid of follow1 and follow2 because of server crash
...
void Cmd_CallVote_f( gentity_t *ent ) {
	...
	//JediDog: Added this to prevent callvote bug
...
void Cmd_CallTeamVote_f( gentity_t *ent ) {
	...
	//JediDog: removed this for what is below
/*	if( strchr( arg1, ';' ) || strchr( arg2, ';' ) ) {
		trap_SendServerCommand( ent-g_entities, "print \"Invalid vote string.\n\"" );
		return;
	}
*/	//JediDog: Added this instead of what is above for callvote bug
	if( strchr(arg1,';') || strchr(arg2,';') || strchr(arg1,'\n') || strchr(arg2,'\n') || strchr(arg1,'\r') || strchr(arg2,'\r')) {
		trap_SendServerCommand(ent-g_entities, "print \"Invalid vote string.\n\"" );
		return;
	}
...
void Cmd_EngageDuel_f(gentity_t *ent)
{
	...
	*/
	...
	//JediDog: Sleeping clients cannot duel challenge
	if (ent->client->pers.Sleep == qtrue)
	{
		return;
	}
...
void ClientCommand( int clientNum ) {
	...
	//JediDog: Begin client commands
		...
		trap_SendServerCommand(ent-g_entities,"print\"^5Info: Anti Flood, Anti Force Crash, Anti Name Crash, Anti Fake Players & Anti MSGBoom. With a few extras.\n\"");
		trap_SendServerCommand(ent-g_entities,"print\"^2Client Commands:\n^5/help (this help menu)\n/sit (sit down)\n\"");
		trap_SendServerCommand(ent-g_entities,"print\"^2Admin Commands:\n^5/rcon sleep <Client Number> (make a client fall to sleep)\n/rcon slap <Client Number> (slap a client)\n/rcon csay <msg> (lets you talk in middle of screen)\n\"");
	}
	else if (Q_stricmp(cmd, "admincmds") == 0) {
		
	}
	else if (Q_stricmp(cmd, "sit") == 0 ) {
//		if (jd_SitEmote.integer == 0) {
//			trap_SendServerCommand(ent-g_entities, va("print\"^5Sit emote is disabled in this server.\n\""));
//			return;
//		}
		if (!g_gametype.integer == GT_TEAM || !g_gametype.integer == GT_FFA) {
			trap_SendServerCommand(ent-g_entities, va("print\"^5Sit emote is disabled in this gametype.\n\""));
			return;
		}
	...
	//JediDog: end of client commands
	/*
