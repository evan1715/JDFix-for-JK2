...
//		G_Printf("serverTime >>>>>\n" );
	...
	if (ent->client->ps.duelInProgress)
	{
		...
		//JediDog: Changed 10000 to 2500 so the time would be less annoying
		ent->client->ps.fd.privateDuelTime = level.time + 2500;
...
void SpectatorClientEndFrame( gentity_t *ent ) {
	...
	// ~Dark
		dTeams[TEAM_RED] = TeamCount(ignoreClientNum, TEAM_RED);
		dTeams[TEAM_BLUE] = TeamCount(ignoreClientNum, TEAM_BLUE);

		clientNum = ent->client->sess.spectatorClient;

		// team follow1 and team follow2 go to whatever clients are playing
		if (clientNum == -1 && dTeams[TEAM_RED] > 0) { // && to prevent a crash when no one is on the specified team.
			clientNum = level.follow1;
		} else if (clientNum == -2 && dTeams[TEAM_BLUE] > 0) { // && to prevent a crash when no one is on the specified team.
