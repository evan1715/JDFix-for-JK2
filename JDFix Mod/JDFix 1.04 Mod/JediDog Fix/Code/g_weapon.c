...
======================================================================
*/
//JediDog: original wp_FireTurretMissle
/*
//---------------------------------------------------------
...
*/
//JediDog: added this for turret/sentry bug (Ouned's fix)
//---------------------------------------------------------
void WP_FireTurretMissile( gentity_t *ent, vec3_t start, vec3_t dir, qboolean altFire, int damage, int velocity, int mod, gentity_t *ignore )
//---------------------------------------------------------
{
  ...
  if (mod == MOD_SENTRY)
  {
    for ( e = 0 ; e <= 999 ; e++ ) 
	  {
		  depp = &g_entities[e];
      if ( depp->s.weapon == WP_TURRET || depp->isseeker == qtrue )
      {
        vadder++;
      }
    }
...
//JediDog: original wp_FireGenericBlasterMissle
/*
//---------------------------------------------------------
...
//---------------------------------------------------------
{
   ...
   for ( e = 0 ; e <= 999 ; e++ ) 
	 {
		  depp = &g_entities[e];
      if ( depp->isseeker == qtrue || depp->s.weapon == WP_TURRET )
      {
        vadder++;
      }
   }
   ...
   if ( vadder <= 10 )
   {
	   ...
	   missile->dflags = DAMAGE_DEATH_KNOCKBACK;
	   missile->methodOfDeath = mod;
	   // IM A SEEKER DRONE !!!1!1
	   missile->isseeker = qtrue;
	   missile->clipmask = MASK_SHOT | CONTENTS_LIGHTSABER;
	   missile->bounceCount = 8;
	 }
}
