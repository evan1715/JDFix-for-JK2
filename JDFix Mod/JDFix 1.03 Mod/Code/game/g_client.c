...
//	char		*areabits;
	...
	#define jdPBFakeDetection "cl_punkbuster"
	#define jdCLGFakeDetection "cl_guid"
	int i;
	int c;
	int j;
	...
	// get the portless ip Anti Fake Players Core
	...
	if (jd_FakePlayerDetection.integer != 0) {
		if (strcmp(Info_ValueForKey (userinfo, jdPBFakeDetection),"") != 0 || strcmp(Info_ValueForKey (userinfo, jdCLGFakeDetection),"") != 0) {
			if (jdPBFakeDetection || jdCLGFakeDetection) {
				G_LogPrintf("JEDIDOG FIX: The IP Address %s has Fake Player Settings. The client has been denied access.\n", jdPortlessIP);
