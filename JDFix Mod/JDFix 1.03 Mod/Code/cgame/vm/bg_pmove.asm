...
EQI4 $1675
line 4154
;4154:	{//JediDog: Changed backwards back to 1.03's settings
line 4155
;4155:		ps->speed *= 1;
ADDRLP4 36
ADDRFP4 0
...
NEI4 $1720
line 4262
;4262:		{ //JediDog: Changed backwards roll to 1.03's settings
line 4263
;4263:			ps->speed = ps->legsTimer/2.25;
ADDRLP4 72
ADDRFP4 0
INDIRP4
ASGNP4
...
line 4452
;4450:
;4451:	//JediDog: This part is the DFA & Lunge non-pivoting for 1.03
;4452:	if (pm->ps->saberMove == LS_A_JUMP_T__B_ || pm->ps->saberMove == LS_A_LUNGE)
ADDRLP4 56
ADDRGP4 pm
INDIRP4
INDIRP4
...
;4453:	{
line 4454
;4454:		PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
ADDRLP4 60
ADDRGP4 pm
INDIRP4
ASGNP4
...
line 4457
;4456:
;4457:	if ((pm->ps->legsAnim&~ANIM_TOGGLEBIT) == BOTH_KISSER1LOOP ||
ADDRLP4 60
ADDRGP4 pm
INDIRP4
INDIRP4
...
LABELV $1791
line 4459
;4458:		(pm->ps->legsAnim&~ANIM_TOGGLEBIT) == BOTH_KISSEE1LOOP)
;4459:	{
line 4460
;4460:		pm->ps->viewangles[PITCH] = 0;
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 156
...
ASGNF4
line 4461
;4461:		PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
ADDRLP4 64
ADDRGP4 pm
INDIRP4
ASGNP4
...
line 4465
;4463:
;4464:	// update the viewangles
;4465:	PM_UpdateViewAngles( pm->ps, &pm->cmd );
ADDRLP4 64
ADDRGP4 pm
INDIRP4
ASGNP4
...
line 4467
;4466:
;4467:	AngleVectors (pm->ps->viewangles, pml.forward, pml.right, pml.up);
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 156
...
GEI4 $1794
line 4471
;4470:		// not holding jump
;4471:		pm->ps->pm_flags &= ~PMF_JUMP_HELD;
ADDRLP4 68
ADDRGP4 pm
INDIRP4
INDIRP4
...
line 4475
;4473:
;4474:	// decide if backpedaling animations should be used
;4475:	if ( pm->cmd.forwardmove < 0 ) {
ADDRGP4 pm
INDIRP4
CNSTI4 28
ADDP4
...
GEI4 $1796
line 4476
;4476:		pm->ps->pm_flags |= PMF_BACKWARDS_RUN;
ADDRLP4 68
ADDRGP4 pm
INDIRP4
INDIRP4
...
ASGNI4
line 4477
;4477:	} else if ( pm->cmd.forwardmove > 0 || ( pm->cmd.forwardmove == 0 && pm->cmd.rightmove ) ) {
ADDRGP4 $1797
JUMPV
LABELV $1796
ADDRLP4 68
...
LABELV $1800
line 4478
;4478:		pm->ps->pm_flags &= ~PMF_BACKWARDS_RUN;
ADDRLP4 80
ADDRGP4 pm
INDIRP4
INDIRP4
...
line 4487
;4486:
;4487:	if (pm->ps->saberLockTime >= pm->cmd.serverTime)
ADDRLP4 84
ADDRGP4 pm
INDIRP4
ASGNP4
...
line 4494
;4493:
;4494:	if ( pm->ps->pm_type == PM_SPECTATOR ) {
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 4
...
;4499:	}
;4500:
;4501:	if ( pm->ps->pm_type == PM_NOCLIP ) {
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 4
...
;4505:	}
;4506:
;4507:	if (pm->ps->pm_type == PM_FREEZE) {
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 4
...
NEI4 $1809
line 4508
;4508:		return;		// no movement at all
ADDRGP4 $1724
JUMPV
LABELV $1809
line 4511
;4509:	}
;4510:
;4511:	if ( pm->ps->pm_type == PM_INTERMISSION || pm->ps->pm_type == PM_SPINTERMISSION) {
ADDRLP4 88
ADDRGP4 pm
INDIRP4
INDIRP4
...
LABELV $1813
line 4512
;4512:		return;		// no movement at all
ADDRGP4 $1724
JUMPV
LABELV $1811
line 4515
...
line 4521
;4519:
;4520:	// set watertype, and waterlevel
;4521:	PM_SetWaterLevel();
ADDRGP4 PM_SetWaterLevel
CALLV
pop
line 4522
;4522:	pml.previous_waterlevel = pmove->waterlevel;
ADDRGP4 pml+1160
ADDRFP4 0
INDIRP4
CNSTI4 216
...
line 4525
;4523:
;4524:	// set mins, maxs, and viewheight
;4525:	PM_CheckDuck ();
ADDRGP4 PM_CheckDuck
CALLV
pop
...
line 4530
;4529:
;4530:	if ( pm->ps->groundEntityNum != ENTITYNUM_NONE )
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 84
...
;4531:	{//on ground
line 4532
;4532:		pm->ps->fd.forceJumpZStart = 0;
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 1092
...
line 4541
;4540:
;4541:	if (pm->ps->pm_type == PM_FLOAT)
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 4
...
;4546:	{
line 4547
;4547:		if (pm->ps->pm_flags & PMF_TIME_WATERJUMP) {
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 12
...
EQI4 $1823
line 4548
;4548:			PM_WaterJumpMove();
ADDRGP4 PM_WaterJumpMove
CALLV
pop
line 4549
;4549:		} else if ( pm->waterlevel > 1 ) {
ADDRGP4 $1824
JUMPV
LABELV $1823
ADDRGP4 pm
...
line 4564
;4562:
;4563:	// set groundentity, watertype, and waterlevel
;4564:	PM_GroundTrace();
ADDRGP4 PM_GroundTrace
CALLV
pop
line 4565
;4565:	PM_SetWaterLevel();
ADDRGP4 PM_SetWaterLevel
CALLV
pop
line 4567
;4566:
;4567:	if (pm->cmd.forcesel != -1 && (pm->ps->fd.forcePowersKnown & (1 << pm->cmd.forcesel)))
ADDRLP4 96
ADDRGP4 pm
INDIRP4
ASGNP4
...
;4568:	{
line 4569
;4569:		pm->ps->fd.forcePowerSelected = pm->cmd.forcesel;
ADDRLP4 104
ADDRGP4 pm
INDIRP4
ASGNP4
...
LABELV $1830
line 4571
;4571:	if (pm->cmd.invensel != -1 && (pm->ps->stats[STAT_HOLDABLE_ITEMS] & (1 << pm->cmd.invensel)))
ADDRLP4 104
ADDRGP4 pm
INDIRP4
ASGNP4
...
;4572:	{
line 4573
;4573:		pm->ps->stats[STAT_HOLDABLE_ITEM] = BG_GetItemIndexByTag(pm->cmd.invensel, IT_HOLDABLE);
ADDRLP4 112
ADDRGP4 pm
INDIRP4
ASGNP4
...
line 4582
;4580:
;4581:	// footstep events / legs animations
;4582:	PM_Footsteps();
...
line 4585
;4583:
;4584:	// entering / leaving water splashes
;4585:	PM_WaterEvents();
...
line 4588
;4586:
;4587:	// snap some parts of playerstate to save network bandwidth
;4588:	trap_SnapVector( pm->ps->velocity );
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 32
...
;4599:Pmove
;4600:
;4601:Can be called by either the server or the client
;4602:================
;4603:*/
;4604:void Pmove (pmove_t *pmove) {
line 4607
;4605:	int			finalTime;
;4606:
;4607:	finalTime = pmove->cmd.serverTime;
ADDRLP4 0
ADDRFP4 0
INDIRP4
CNSTI4 4
...
line 4609
;4608:
;4609:	if ( finalTime < pmove->ps->commandTime ) {
ADDRLP4 0
INDIRI4
ADDRFP4 0
INDIRP4
...
;4611:	}
;4612:
;4613:	if ( finalTime > pmove->ps->commandTime + 1000 ) {
ADDRLP4 0
INDIRI4
ADDRFP4 0
INDIRP4
...
LEI4 $1839
line 4614
;4614:		pmove->ps->commandTime = finalTime - 1000;
ADDRFP4 0
INDIRP4
INDIRP4
ADDRLP4 0
...
line 4617
;4616:
;4617:	if (pmove->ps->fallingToDeath)
ADDRFP4 0
INDIRP4
INDIRP4
CNSTI4 1352
...
line 4625
;4624:
;4625:	pmove->ps->pmove_framecount = (pmove->ps->pmove_framecount+1) & ((1<<PS_PMOVEFRAMECOUNTBITS)-1);
ADDRLP4 4
ADDRFP4 0
INDIRP4
INDIRP4
...
line 4629
;4626:
;4627:	// chop the move up if it is too long, to prevent framerate
;4628:	// dependent behavior
;4629:	while ( pmove->ps->commandTime != finalTime ) {
line 4632
;4630:		int		msec;
;4631:
;4632:		msec = finalTime - pmove->ps->commandTime;
ADDRLP4 8
ADDRLP4 0
INDIRI4
ADDRFP4 0
...
LABELV $1847
line 4644
;4644:		pmove->cmd.serverTime = pmove->ps->commandTime + msec;
ADDRLP4 12
ADDRFP4 0
INDIRP4
ASGNP4
...
line 4647
;4646:
;4647:		if ( pmove->ps->pm_flags & PMF_JUMP_HELD ) {
ADDRFP4 0
INDIRP4
INDIRP4
CNSTI4 12
