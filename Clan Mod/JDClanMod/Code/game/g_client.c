...
void ClientUserinfoChanged( int clientNum ) {
	...
	//JediDog: This is the Anti Flood
	if (jd_antiFlood.integer != 0) {
...
//	char		*areabits;
	...
	//JediDog: Added log for full userinfo including IP
	...
	if ( G_FilterPacket( tmpIP ) ) {
		G_LogPrintf("IP banned: Access denied to <%s>\n", tmpIP);//JediDog: Added log
		return "Banned.";
	}
	...
	if ( !( ent->r.svFlags & SVF_BOT ) && !isBot && g_needpass.integer ) {
		...
		if ( g_password.string[0] && Q_stricmp( g_password.string, "none" ) &&
			...
			G_LogPrintf("Invalid password: Access denied to client %i: Entered password <%s> instead of <%s>\n",clientNum, value, g_password.string);//JediDog: Added log
			return sTemp;// return "Invalid password";
		}
	}
	//JEDIDOG: BEGINNING OF JEDIDOG'S FIXES
