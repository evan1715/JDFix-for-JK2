...
EQI4 $1679
line 4163
;4163:	{//JediDog: Changed backwards back to 1.03's settings
line 4164
;4164:		ps->speed *= 1;
ADDRLP4 36
ADDRFP4 0
...
line 4461
;4459:
;4460:	//JediDog: This part is the DFA & Lunge non-pivoting for 1.03
;4461:	if (pm->ps->saberMove == LS_A_JUMP_T__B_ || pm->ps->saberMove == LS_A_LUNGE)
ADDRLP4 56
ADDRGP4 pm
INDIRP4
INDIRP4
...
;4462:	{
line 4463
;4463:		PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
ADDRLP4 60
ADDRGP4 pm
INDIRP4
ASGNP4
...
LABELV $1790
line 4473
;4465:/*	 JediDog: this is the default for locked cameras on bs, dbs and dfa	
;4466:	if (pm->ps->saberMove == LS_A_JUMP_T__B_ || pm->ps->saberMove == LS_A_LUNGE ||
;4467:		pm->ps->saberMove == LS_A_BACK_CR || pm->ps->saberMove == LS_A_BACK ||
;4468:		pm->ps->saberMove == LS_A_BACKSTAB)
;4469:	{
;4470:		PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
;4471:	}
;4472:*/
;4473:	if ((pm->ps->legsAnim&~ANIM_TOGGLEBIT) == BOTH_KISSER1LOOP ||
ADDRLP4 60
ADDRGP4 pm
INDIRP4
INDIRP4
...
LABELV $1795
line 4475
;4474:		(pm->ps->legsAnim&~ANIM_TOGGLEBIT) == BOTH_KISSEE1LOOP)
;4475:	{
line 4476
;4476:		pm->ps->viewangles[PITCH] = 0;
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 156
...
ASGNF4
line 4477
;4477:		PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
ADDRLP4 64
ADDRGP4 pm
INDIRP4
ASGNP4
...
line 4481
;4479:
;4480:	// update the viewangles
;4481:	PM_UpdateViewAngles( pm->ps, &pm->cmd );
ADDRLP4 64
ADDRGP4 pm
INDIRP4
ASGNP4
...
line 4483
;4482:
;4483:	AngleVectors (pm->ps->viewangles, pml.forward, pml.right, pml.up);
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 156
...
GEI4 $1798
line 4487
;4486:		// not holding jump
;4487:		pm->ps->pm_flags &= ~PMF_JUMP_HELD;
ADDRLP4 68
ADDRGP4 pm
INDIRP4
INDIRP4
...
line 4491
;4489:
;4490:	// decide if backpedaling animations should be used
;4491:	if ( pm->cmd.forwardmove < 0 ) {
ADDRGP4 pm
INDIRP4
CNSTI4 28
ADDP4
...
GEI4 $1800
line 4492
;4492:		pm->ps->pm_flags |= PMF_BACKWARDS_RUN;
ADDRLP4 68
ADDRGP4 pm
INDIRP4
INDIRP4
...
ASGNI4
line 4493
;4493:	} else if ( pm->cmd.forwardmove > 0 || ( pm->cmd.forwardmove == 0 && pm->cmd.rightmove ) ) {
ADDRGP4 $1801
JUMPV
LABELV $1800
ADDRLP4 68
...
LABELV $1804
line 4494
;4494:		pm->ps->pm_flags &= ~PMF_BACKWARDS_RUN;
ADDRLP4 80
ADDRGP4 pm
INDIRP4
INDIRP4
...
line 4503
;4502:
;4503:	if (pm->ps->saberLockTime >= pm->cmd.serverTime)
ADDRLP4 84
ADDRGP4 pm
INDIRP4
ASGNP4
...
line 4510
;4509:
;4510:	if ( pm->ps->pm_type == PM_SPECTATOR ) {
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 4
...
;4515:	}
;4516:
;4517:	if ( pm->ps->pm_type == PM_NOCLIP ) {
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 4
...
;4521:	}
;4522:
;4523:	if (pm->ps->pm_type == PM_FREEZE) {
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 4
...
NEI4 $1813
line 4524
;4524:		return;		// no movement at all
ADDRGP4 $1728
JUMPV
LABELV $1813
line 4527
;4525:	}
;4526:
;4527:	if ( pm->ps->pm_type == PM_INTERMISSION || pm->ps->pm_type == PM_SPINTERMISSION) {
ADDRLP4 88
ADDRGP4 pm
INDIRP4
INDIRP4
...
LABELV $1817
line 4528
;4528:		return;		// no movement at all
ADDRGP4 $1728
JUMPV
LABELV $1815
line 4531
...
line 4537
;4535:
;4536:	// set watertype, and waterlevel
;4537:	PM_SetWaterLevel();
ADDRGP4 PM_SetWaterLevel
CALLV
pop
line 4538
;4538:	pml.previous_waterlevel = pmove->waterlevel;
ADDRGP4 pml+1160
ADDRFP4 0
INDIRP4
CNSTI4 216
...
line 4541
;4539:
;4540:	// set mins, maxs, and viewheight
;4541:	PM_CheckDuck ();
ADDRGP4 PM_CheckDuck
CALLV
pop
...
line 4546
;4545:
;4546:	if ( pm->ps->groundEntityNum != ENTITYNUM_NONE )
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 84
...
;4547:	{//on ground
line 4548
;4548:		pm->ps->fd.forceJumpZStart = 0;
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 1092
...
line 4557
;4556:
;4557:	if (pm->ps->pm_type == PM_FLOAT)
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 4
...
;4562:	{
line 4563
;4563:		if (pm->ps->pm_flags & PMF_TIME_WATERJUMP) {
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 12
...
EQI4 $1827
line 4564
;4564:			PM_WaterJumpMove();
ADDRGP4 PM_WaterJumpMove
CALLV
pop
line 4565
;4565:		} else if ( pm->waterlevel > 1 ) {
ADDRGP4 $1828
JUMPV
LABELV $1827
ADDRGP4 pm
...
line 4580
;4578:
;4579:	// set groundentity, watertype, and waterlevel
;4580:	PM_GroundTrace();
ADDRGP4 PM_GroundTrace
CALLV
pop
line 4581
;4581:	PM_SetWaterLevel();
ADDRGP4 PM_SetWaterLevel
CALLV
pop
line 4583
;4582:
;4583:	if (pm->cmd.forcesel != -1 && (pm->ps->fd.forcePowersKnown & (1 << pm->cmd.forcesel)))
ADDRLP4 96
ADDRGP4 pm
INDIRP4
ASGNP4
...
;4584:	{
line 4585
;4585:		pm->ps->fd.forcePowerSelected = pm->cmd.forcesel;
ADDRLP4 104
ADDRGP4 pm
INDIRP4
ASGNP4
...
LABELV $1834
line 4587
;4587:	if (pm->cmd.invensel != -1 && (pm->ps->stats[STAT_HOLDABLE_ITEMS] & (1 << pm->cmd.invensel)))
ADDRLP4 104
ADDRGP4 pm
INDIRP4
ASGNP4
...
;4588:	{
line 4589
;4589:		pm->ps->stats[STAT_HOLDABLE_ITEM] = BG_GetItemIndexByTag(pm->cmd.invensel, IT_HOLDABLE);
ADDRLP4 112
ADDRGP4 pm
INDIRP4
ASGNP4
...
line 4598
;4596:
;4597:	// footstep events / legs animations
;4598:	PM_Footsteps();
...
line 4601
;4599:
;4600:	// entering / leaving water splashes
;4601:	PM_WaterEvents();
...
line 4604
;4602:
;4603:	// snap some parts of playerstate to save network bandwidth
;4604:	trap_SnapVector( pm->ps->velocity );
ADDRGP4 pm
INDIRP4
INDIRP4
CNSTI4 32
...
;4615:Pmove
;4616:
;4617:Can be called by either the server or the client
;4618:================
;4619:*/
;4620:void Pmove (pmove_t *pmove) {
line 4623
;4621:	int			finalTime;
;4622:
;4623:	finalTime = pmove->cmd.serverTime;
ADDRLP4 0
ADDRFP4 0
INDIRP4
CNSTI4 4
...
line 4625
;4624:
;4625:	if ( finalTime < pmove->ps->commandTime ) {
ADDRLP4 0
INDIRI4
ADDRFP4 0
INDIRP4
...
;4627:	}
;4628:
;4629:	if ( finalTime > pmove->ps->commandTime + 1000 ) {
ADDRLP4 0
INDIRI4
ADDRFP4 0
INDIRP4
...
LEI4 $1843
line 4630
;4630:		pmove->ps->commandTime = finalTime - 1000;
ADDRFP4 0
INDIRP4
INDIRP4
ADDRLP4 0
...
line 4633
;4632:
;4633:	if (pmove->ps->fallingToDeath)
ADDRFP4 0
INDIRP4
INDIRP4
CNSTI4 1352
...
line 4641
;4640:
;4641:	pmove->ps->pmove_framecount = (pmove->ps->pmove_framecount+1) & ((1<<PS_PMOVEFRAMECOUNTBITS)-1);
ADDRLP4 4
ADDRFP4 0
INDIRP4
INDIRP4
...
line 4645
;4642:
;4643:	// chop the move up if it is too long, to prevent framerate
;4644:	// dependent behavior
;4645:	while ( pmove->ps->commandTime != finalTime ) {
line 4648
;4646:		int		msec;
;4647:
;4648:		msec = finalTime - pmove->ps->commandTime;
ADDRLP4 8
ADDRLP4 0
INDIRI4
ADDRFP4 0
...
LABELV $1851
line 4660
;4660:		pmove->cmd.serverTime = pmove->ps->commandTime + msec;
ADDRLP4 12
ADDRFP4 0
INDIRP4
ASGNP4
...
line 4663
;4662:
;4663:		if ( pmove->ps->pm_flags & PMF_JUMP_HELD ) {
ADDRFP4 0
INDIRP4
INDIRP4
CNSTI4 12
