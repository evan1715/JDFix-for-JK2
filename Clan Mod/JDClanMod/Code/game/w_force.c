...
void WP_InitForcePowers( gentity_t *ent )
{
	...
	*/
	...
	//JediDog: anti force crash
	if (jd_antiForceCrash.integer != 0) {
...
void ForceHeal( gentity_t *self )
{
	...
	{
		self->health += 25; //JediDog: This was 25, for 1.04, but we changed it back to 1.03's settings... +50
	...
	{
		self->health += 10; //JediDog: This was 10 for 1.04, but we changed it back to +20
	...
	{
		self->health += 5; //JediDog: This was 5 for 1.03, but we changed it back to +10.
				...
				if (modPowerLevel > otherPushPower && push_list[x]->client)
				{
						...
						push_list[x]->client->ps.forceHandExtend != HANDEXTEND_KNOCKDOWN)
					{
						...
						{ //can only do a knockdown if fairly close
							push_list[x]->client->ps.forceHandExtend = HANDEXTEND_KNOCKDOWN;
							push_list[x]->client->ps.forceHandExtendTime = level.time + 700;//JediDog: get up timing
