...
*/
	...
	if (g_dedicated.integer) {
        // JediDog: Better looking when server talks.
		if (Q_stricmp (cmd, "say") == 0) {
			trap_SendServerCommand( -1, va("print \"^5[^7Server^5]^7: %s\n\"", ConcatArgs(1) ) );
			return qtrue;
        }// JediDog: added csay to make msgs appear in the middle of the screen without the "[Server]" infront of it.
        else if (Q_stricmp (cmd, "csay") == 0) {
