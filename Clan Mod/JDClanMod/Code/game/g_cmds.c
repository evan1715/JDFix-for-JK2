...
char	*ConcatArgs( int start ) {
	...
	for ( i = start ; i < c ; i++ ) {
		...
		tlen = strlen( arg );
		//JediDog: added this line below for extra anti msgboom protection
		if ( len + tlen >= 850 ) {
			break;
		}
...
void ClientCommand( int clientNum ) {
	...
	{ // teleport to specific location
		...
		if ( trap_Argc() == 3 )
		{
			...
			if (client_id2 >= MAX_CLIENTS || client_id < 0)
			{
				 trap_SendServerCommand( ent-g_entities, va("print \"Bad client ID for %s\n\"", arg1 ) );
				 return;
			}
	...
	//cm EMOTES
		...
		else if (Q_stricmp(cmd, "blah") == 0)
		{
			StandardSetBodyAnim(ent, 428, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD|SETANIM_FLAG_HOLDLESS);
		}
