...
void ClientUserinfoChanged( int clientNum ) {
	...
	if (ent->client->TimeReswitch < level.time && ent->client->TimeReswitch > 0 && ent->client->TimeReswitch > level.time-400) {
...
void ClientBegin( int clientNum, qboolean allowTeamReset ) {
	...
	if (client->sess.sessionTeam != TEAM_SPECTATOR){
		trap_SendServerCommand( ent-g_entities, va("print \"\n\n^5Hi. This is ^2JediDog's ^5edited version of NeWaGe's Clan Mod v2.02... It's improved with security and bug fixes.\n^5For help on this specific mod go to http://jedidog.freeforums.org\n\n\""));
